...
$ret=mysqli_query($con,"SELECT * FROM worker WHERE email='".$_POST['email']."' and password='".md5($password)."' ");
...
if($num['status']=='notverified'){
    echo "<script>window.alert('Admin not verified yet!');</script>";
} else{
echo "<script>window.alert('login succesfull');</script>";
	echo "<script>window.location.assign('worker/dashboard.php');</script>";}
}
else{
      ...
      <div class="max-w-md mx-auto">
        <form class="text-left" method='post'>
          ...
          <p class="text-gray-700 mt-2">
            <a href="workerforgotpassword.php" class="text-green-600 hover:underline">Forgot Password?</a>
          </p>
        </form>
        <p class="text-gray-700 mt-4">
          Don't have an account? <a href="workerregistration.php" class="text-green-600 hover:underline">Sign up here</a>.
        </p>
      </div>
    </div>
  </section>
