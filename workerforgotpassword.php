...
{
$ret=mysqli_query($con,"SELECT * FROM worker WHERE email='".$_POST['email']."' ");
...
if($num>0)
{
    ...
    $body ="http://localhost/cleankerala/workerresetpassword.php?id=".$n['id']."&key=".$n['password'];
    $headers = "From:@gmail.com";
    if (mail($to_email, $subject, $body, $headers)) {

        echo "<script>alert('Password reset link send to your email id ');</script>";
        echo "<script>window.location.assign('workerforgotpassword.php');</script>";
    }
    else {
