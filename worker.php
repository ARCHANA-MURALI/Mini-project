...
if(isset($_GET['delete'])){
    $id=$_GET['id'];
    $query=mysqli_query($con,"DELETE from worker WHERE id = '$id'");
    if($query){
      echo "<script>alert('Successfully Deleted');</script>";
      echo "<script>window.location.assign('worker.php');</script>"; 
    }
  }
  if(isset($_GET['update'])){
    $id=$_GET['update'];
    $query=mysqli_query($con,"UPDATE worker set status='verified' WHERE id = '$id'");
    if($query){
                ...
                <table class="w-full bg-white border-collapse border border-gray-300">
                    <thead>
                        <tr class="bg-gray-200">
                            ...
                            <th class="p-3 font-semibold text-left">Id Proof</th>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) { ?>
        ...
        <td class='p-3'><img  class='w-44 h-44' src='../worker/idproof/<?php echo $row["idproof"]; ?>'> </td>
        <td class='p-3'> <?php echo $row["category"]; ?> </td>
        <td class='p-3'> <?php echo $row["date"]; ?> </td>
        <td class='p-3'> <?php echo $row["status"]; ?> </td>
      
     <td class='p-3'>
        <?php if($row['status']=='notverified') { ?>      <a href='worker.php?update=<?php echo $row['id'] ; ?>' onClick="return confirm('Are you sure you want to verify?')"
             class='text-blue-500 hover:underline mr-2'>Verify</a><?php }  ?>
                <a href='worker.php?id=<?php echo $row['id'] ; ?>&delete=delete' onClick="return confirm('Are you sure you want to delete?')" class='text-red-500 hover:underline'>Delete</a>
            </td>
        </tr>
  <?php  }
}
